...
function Teams({ teams, achievements, hackers, topics }) {
    ...
    const renderedTeams = rankedTeams.map(({ name, slackChannel, score, rank, showRank, achievements, hackers, topic }) => (
    ...
    return (
        <div style={{ display: "flex" }}>
            <div style={{ margin: "10px" }}>
                {renderedTeams.map((team, index) => index < renderedTeams.length / 2 ? team : null)}
            </div>
            <div style={{ margin: "10px" }}>
                {renderedTeams.map((team, index) => index >= renderedTeams.length / 2 ? team : null)}
            </div>
